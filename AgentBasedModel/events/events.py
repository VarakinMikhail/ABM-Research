...
from AgentBasedModel.agents import Trader, Universalist, Fundamentalist, Chartist, MarketMaker
...
class InformationalShock(Event):
    def __init__(self, it, lag: int):
        super().__init__(it)
        self.lag = lag

    def __repr__(self):
        return f'informational shock (it={self.it}, lag={self.lag})'

    def call(self, it: int):
        if super().call(it):
            return
        for trader in self.simulator.traders:
            if type(trader) in (Universalist, Fundamentalist, Chartist):
                trader.lag = self.lag
