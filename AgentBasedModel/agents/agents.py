...
class Trader:
    ...
    def __init__(self, market: ExchangeAgent, cash: float or int, assets: int = 0):
        ...
        self.cash = cash
        self.assets = assets
        self.lag = 0 #standard value (no lag)
...
class Fundamentalist(Trader):
    ...
    def call(self, info):
        ...
        p = info.prices[-(self.lag + 1)] if len(info.prices) > self.lag + 1 else info.prices[0]
        spread = self.market.spread()
...
class Chartist(Trader):
    ...
    def change_sentiment(self, info, a1=1, a2=1, v1=.1):
        ...
        p = info.prices[-(self.lag + 1)] if len(info.prices) > self.lag + 1 else info.prices[0]
        pp = info.prices[-(self.lag + 2)] if len(info.prices) > self.lag + 2 else info.prices[0]

        dp = p - pp if len(info.prices) > self.lag + 2 else 0  # price derivative
...
class Universalist(Fundamentalist, Chartist):
    ...
    def call(self, info):
        ...
        if self.type == 'Chartist':
            Chartist.call(self, info)
        elif self.type == 'Fundamentalist':
            Fundamentalist.call(self, info)
